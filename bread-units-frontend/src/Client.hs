...
productCarbs' = client serverAPI (Proxy :: Proxy m) (Proxy :: Proxy ()) (pure (BasePath "/"))
...
productCarbs name = do
  ...
  widgetHold (return ()) $ ffor respE $ \case
    ResponseSuccess _ _ _ -> return ()
    ResponseFailure _ msg _ -> danger msg
    RequestFailure _ msg -> danger msg
